...
async function authenticate(req) {
  ...
  return {
    ...
    adminClaim: payload.admin === true || payload.role === 'admin'
  };
}

function isAdmin(identity) {
  return Boolean(identity?.adminClaim || configuredAdminUids.has(identity?.uid) || configuredAdminEmails.has(String(identity?.email || '').toLowerCase()));
}

async function requireAdmin(req) {
  const identity = await authenticate(req);
  if (!isAdmin(identity)) throw Object.assign(new Error('ADMIN_REQUIRED'), { statusCode: 403 });
  return identity;
}
...
function cleanStatus(value) {
  return ['published', 'needs-review', 'hidden'].includes(value) ? value : 'published';
}

function safeOptions(value) {
  if (!Array.isArray(value)) return [];
  return value.slice(0, 8).map(option => cleanText(option, 1200)).filter(Boolean);
}
...
async function loadManifest() {
  ...
  manifestCache = await loadJsonFile('data/exams/manifest.json');
...
async function loadBaseBank(subjectId) {
  ...
  const file = manifest.subjects?.[subjectId]?.file || `${subjectId}.json`;
  const rows = await loadJsonFile(`data/exams/${file}`);
...
async function overrideRows(subjectId = '') {
  ...
  const result = subjectId
    ? await pool.query('SELECT * FROM neon_question_overrides WHERE subject_id=$1', [subjectId])
    : await pool.query('SELECT * FROM neon_question_overrides');
  return result.rows;
}
...
function applyOverride(question, override) {
  ...
  merged.adminStatus = cleanStatus(override.review_status);
  merged.adminEdited = true;
  merged.adminUpdatedAt = override.updated_at || null;
...
async function effectiveBank(subjectId, { includeHidden = false } = {}) {
  ...
  return base
    .map(question => applyOverride(question, overrides.get(String(question.id))))
    .filter(question => includeHidden || question.active !== false);
}
...
function publicQuestion(question) {
  const { adminStatus, adminEdited, adminUpdatedAt, ...safe } = question;
  return safe;
}
...
async function serveQuestionData(req, res, requestPath) {
  ...
  const revision = await revisionToken();
  if (revision === 'base' || revision === '0') return false;
  const rows = (await effectiveBank(match[1])).map(publicQuestion);
...
function trigrams(value) {
  ...
  for (let index = 0; index < text.length - 2; index += 1) values.add(text.slice(index, index + 3));
  return values;
}
...
function numberSignature(value) {
  return [...normalizeArabic(value).matchAll(/\d+(?:\.\d+)?/g)].map(match => match[0]).join('|');
}

async function potentialDuplicates() {
  const revision = await revisionToken();
  const cacheKey = String(revision);
  if (duplicateCache.key === cacheKey && duplicateCache.expiresAt > Date.now()) return duplicateCache.rows;
  const pairs = [];
  for (const subjectId of subjects) {
    const rows = await effectiveBank(subjectId, { includeHidden: true });
    for (let leftIndex = 0; leftIndex < rows.length; leftIndex += 1) {
      const left = rows[leftIndex];
      const leftText = normalizeArabic(left.q);
      if (leftText.length < 18) continue;
      for (let rightIndex = leftIndex + 1; rightIndex < rows.length; rightIndex += 1) {
        const right = rows[rightIndex];
        const rightText = normalizeArabic(right.q);
        ...
        const similarity = dice(left.q, right.q);
        if (similarity < 0.88) continue;
        pairs.push({
          subjectId,
          subjectTitle: subjectTitles[subjectId],
          similarity: Math.round(similarity * 1000) / 10,
          left: { id: String(left.id), q: left.q, status: left.adminStatus || 'published' },
          right: { id: String(right.id), q: right.q, status: right.adminStatus || 'published' }
        });
      }
    }
  }
  pairs.sort((a, b) => b.similarity - a.similarity);
  duplicateCache = { key: cacheKey, expiresAt: Date.now() + 5 * 60_000, rows: pairs.slice(0, 300) };
...
async function dashboard() {
  ...
  for (const subjectId of subjects) {
    const rows = await effectiveBank(subjectId, { includeHidden: true });
    const visible = rows.filter(row => row.active !== false).length;
    const hiddenCount = rows.length - visible;
    const reviewCount = rows.filter(row => row.adminStatus === 'needs-review').length;
    total += rows.length;
    active += visible;
    hidden += hiddenCount;
  ...
  return {
    ok: true,
    totals: { total, active, hidden, needsReview, openReports: Number(reportCounts.new || 0) + Number(reportCounts.reviewing || 0), potentialDuplicates: duplicates.length },
    reportCounts,
    bySubject,
    revision: await revisionToken()
  };
}

async function listQuestions(url) {
  const subjectFilter = url.searchParams.get('subject') || 'all';
  const search = normalizeArabic(url.searchParams.get('q') || '');
  const status = url.searchParams.get('status') || 'all';
  const page = Math.max(1, Math.trunc(Number(url.searchParams.get('page') || 1)));
  const pageSize = Math.max(10, Math.min(100, Math.trunc(Number(url.searchParams.get('pageSize') || 40))));
  const selectedSubjects = subjectFilter === 'all' ? subjects : subjects.includes(subjectFilter) ? [subjectFilter] : [];
  ...
  for (const subjectId of selectedSubjects) {
    rows.push(...(await effectiveBank(subjectId, { includeHidden: true })).map(question => ({ ...question, subjectId, subjectTitle: subjectTitles[subjectId] })));
  }
  rows = rows.filter(question => {
    if (status !== 'all' && (question.adminStatus || 'published') !== status) return false;
    if (!search) return true;
    return normalizeArabic(`${question.id} ${question.q} ${question.category || ''} ${question.explain || ''}`).includes(search);
  });
  ...
  const items = rows.slice(start, start + pageSize).map(question => ({
    ...question,
    openReports: reportMap.get(`${question.subjectId}|${question.id}`) || 0
  }));
...
async function updateQuestion(identity, body) {
  await ensureSchema();
  const subjectId = cleanId(body?.subjectId, 100);
  const questionId = cleanId(body?.questionId, 220);
  if (!subjects.includes(subjectId) || !questionId) throw Object.assign(new Error('INVALID_QUESTION_ID'), { statusCode: 400 });
  const base = (await loadBaseBank(subjectId)).find(row => String(row.id) === questionId);
  if (!base) throw Object.assign(new Error('QUESTION_NOT_FOUND'), { statusCode: 404 });
  const existingResult = await pool.query('SELECT * FROM neon_question_overrides WHERE subject_id=$1 AND question_id=$2', [subjectId, questionId]);
  const existing = existingResult.rows[0] || null;
  const before = applyOverride(base, existing);
  const incoming = body?.patch && typeof body.patch === 'object' ? body.patch : {};
  const options = safeOptions(incoming.options ?? before.options);
  const answer = Math.trunc(Number(incoming.answer ?? before.answer));
  ...
  const patch = {
    q: cleanText(incoming.q ?? before.q, 5000),
    passage: cleanText(incoming.passage ?? before.passage, 8000),
    options,
    answer,
    explain: cleanText(incoming.explain ?? before.explain, 8000),
    category: cleanText(incoming.category ?? before.category, 180),
    level: cleanText(incoming.level ?? before.level, 80),
    active: incoming.active !== undefined ? Boolean(incoming.active) : before.active !== false
  };
  if (!patch.q) throw Object.assign(new Error('QUESTION_TEXT_REQUIRED'), { statusCode: 400 });
  const reviewStatus = cleanStatus(body?.reviewStatus || existing?.review_status || 'published');
  if (reviewStatus === 'hidden') patch.active = false;
  const result = await pool.query(`
    INSERT INTO neon_question_overrides (subject_id, question_id, patch, review_status, updated_by, updated_at)
    VALUES ($1,$2,$3::jsonb,$4,$5,NOW())
    ON CONFLICT (subject_id, question_id) DO UPDATE SET patch=EXCLUDED.patch, review_status=EXCLUDED.review_status, updated_by=EXCLUDED.updated_by, updated_at=NOW()
    RETURNING *
  `, [subjectId, questionId, JSON.stringify(patch), reviewStatus, identity.uid]);
  const after = applyOverride(base, result.rows[0]);
  await audit(identity, 'question.update', 'question', `${subjectId}:${questionId}`, before, after);
...
async function resetQuestion(identity, body) {
  ...
  const base = (await loadBaseBank(subjectId)).find(row => String(row.id) === questionId);
  if (!base) throw Object.assign(new Error('QUESTION_NOT_FOUND'), { statusCode: 404 });
  const existing = await pool.query('SELECT * FROM neon_question_overrides WHERE subject_id=$1 AND question_id=$2', [subjectId, questionId]);
  const before = applyOverride(base, existing.rows[0]);
  await pool.query('DELETE FROM neon_question_overrides WHERE subject_id=$1 AND question_id=$2', [subjectId, questionId]);
  await audit(identity, 'question.reset', 'question', `${subjectId}:${questionId}`, before, base);
  duplicateCache.expiresAt = 0;
...
async function listReports(url) {
  ...
  const conditions = [];
  const values = [];
  if (status === 'open') conditions.push(`status IN ('new','reviewing')`);
  else if (status !== 'all') { values.push(status); conditions.push(`status=$${values.length}`); }
  if (subject !== 'all') { values.push(subject); conditions.push(`subject_id=$${values.length}`); }
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const countResult = await pool.query(`SELECT COUNT(*)::int AS count FROM neon_question_reports ${where}`, values);
  values.push(pageSize, (page - 1) * pageSize);
  const result = await pool.query(`
    SELECT id, firebase_uid, question_id, subject_id, reason, note, question_text, status, admin_note, resolved_by, resolved_at, created_at
    FROM neon_question_reports ${where}
    ORDER BY CASE status WHEN 'new' THEN 0 WHEN 'reviewing' THEN 1 ELSE 2 END, created_at DESC
    LIMIT $${values.length - 1} OFFSET $${values.length}
  `, values);
  const total = Number(countResult.rows[0]?.count || 0);
  return { ok: true, page, pageSize, total, pages: Math.max(1, Math.ceil(total / pageSize)), items: result.rows };
}

async function updateReport(identity, body) {
  await ensureSchema();
  const id = Math.trunc(Number(body?.id));
  const status = ['new', 'reviewing', 'resolved', 'dismissed'].includes(body?.status) ? body.status : 'reviewing';
  if (!Number.isInteger(id) || id <= 0) throw Object.assign(new Error('INVALID_REPORT_ID'), { statusCode: 400 });
  const beforeResult = await pool.query('SELECT * FROM neon_question_reports WHERE id=$1', [id]);
  if (!beforeResult.rows[0]) throw Object.assign(new Error('REPORT_NOT_FOUND'), { statusCode: 404 });
  const resolved = ['resolved', 'dismissed'].includes(status);
  const result = await pool.query(`
    UPDATE neon_question_reports SET status=$2, admin_note=$3, resolved_by=$4,
      resolved_at=CASE WHEN $5 THEN NOW() ELSE NULL END
    WHERE id=$1 RETURNING *
  `, [id, status, cleanText(body?.adminNote, 1500), identity.uid, resolved]);
  await audit(identity, 'report.update', 'question-report', String(id), beforeResult.rows[0], result.rows[0]);
  return { ok: true, report: result.rows[0] };
}

async function listAudit(url) {
  await ensureSchema();
  const limit = Math.max(10, Math.min(200, Math.trunc(Number(url.searchParams.get('limit') || 80))));
  const result = await pool.query(`SELECT id, firebase_uid, admin_email, action, entity_type, entity_id, before_state, after_state, created_at FROM neon_admin_audit ORDER BY created_at DESC LIMIT $1`, [limit]);
...
export async function handleAdminDashboardApi(req, res, requestPath, readJsonBody) {
  try {
    if (await serveQuestionData(req, res, requestPath)) return true;
  } catch (error) {
    console.error('Question data overlay error:', error?.message || error);
    return false;
  }
  ...
  try {
    ...
    if (requestPath === '/api/admin/status' && req.method === 'GET') {
      json(res, 200, { ok: true, configured: Boolean(pool), authorized: true, identity: { uid: identity.uid, email: identity.email, name: identity.name }, features: ['questions', 'reports', 'duplicates', 'audit', 'live-overrides'] }, cors);
      return true;
    }
    await ensureSchema();
    ...
    console.error('Admin dashboard API error:', failure.code, error?.message || error);
    json(res, failure.status, { error: failure.code, message: failure.message }, cors);
  }
  return true;
}

export async function closeAdminDashboardDatabase() {
